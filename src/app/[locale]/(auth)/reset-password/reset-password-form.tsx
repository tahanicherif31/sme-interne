...
const ResetPasswordForm = () => {
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: api.auth.confirmForgotPassword().key(),
    mutationFn: api.auth.confirmForgotPassword().fn,
    onSuccess: () => {
      toast.success(t("common.resetPasswordSuccess"));
  ...
  const onSubmit: SubmitHandler<ResetPasswordFormType> = ({
    ...
    if (!email) {
      toast.error(t("common.missingResetParams"));
      return;
    }
...
const inputs: FieldProps<ResetPasswordFormType>[] = [
  {
    name: "code",
